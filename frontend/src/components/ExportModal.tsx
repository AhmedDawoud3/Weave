      ...
      >
        ...
        <div className="flex flex-col gap-1 mb-6 shrink-0">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] text-primary font-medium uppercase tracking-wide">Model Exporter</span>
          </div>
          <h2 className="text-xl font-semibold text-foreground">Export compiled artifacts</h2>
        ...
        <div className="flex gap-4 border-b border-border mb-6 shrink-0">
          <button
            ...
            className={`flex items-center gap-2 text-sm font-medium pb-3 border-b-2 transition-all cursor-pointer ${
          ...
          >
            <FolderArchive size={14} /> Standalone project
          </button>
        </div>
        ...
        {(activeTab === 'onnx' || activeTab === 'torchscript') && (
          <div className="flex items-center gap-3 bg-background border border-border px-4 py-2.5 rounded-xl mb-4 shrink-0 text-xs select-none">
            <span className="font-medium text-primary tracking-wide text-[11px] shrink-0">
              Compilation Shape:
            </span>
            <input
              type="text"
          ...
          {activeTab === 'pytorch' ? (
            loading ? (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-muted-foreground">
                ...
                <p className="text-[11px] font-medium text-muted-foreground">Synthesizing PyTorch module...</p>
              </div>
            ) : (
            ...
            <div className="h-full flex flex-col items-center justify-center gap-4 text-center p-6 max-w-md mx-auto">
              ...
              <div>
                <h3 className="text-sm font-semibold mb-2">Export ONNX binary graph</h3>
              ...
              {onnxStatus === 'compiling' && (
                <div className="flex items-center gap-2 text-[11px] font-medium text-primary animate-pulse">
                  <Loader2 size={12} className="animate-spin" /> Compiling ONNX structure...
                </div>
              )}

              {onnxStatus === 'success' && (
                <div className="text-[11px] font-medium text-weave-teal">
                  🎉 Graph successfully compiled & download initiated!
                </div>
              )}

              {onnxStatus === 'error' && (
                <div className="text-[11px] font-medium text-red-400 bg-red-500/10 border border-red-500/20 p-2.5 rounded-lg">
            ...
            <div className="h-full flex flex-col items-center justify-center gap-4 text-center p-6 max-w-md mx-auto">
              ...
              <div>
                <h3 className="text-sm font-semibold mb-2">Export TorchScript binary</h3>
            ...
            <div className="h-full flex flex-col items-center justify-center gap-4 text-center p-6 max-w-md mx-auto animate-fade-in">
              ...
              <div>
                <h3 className="text-sm font-semibold mb-2">Export standalone PyTorch project</h3>
              ...
              <Button
                ...
                className="mt-2 bg-primary hover:brightness-110 text-primary-foreground font-extrabold px-6 rounded-xl flex items-center gap-2 cursor-pointer"
              >
                <Download size={14} /> Generate & download project
              </Button>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end gap-4 shrink-0">
          ...
          {activeTab === 'pytorch' && (
            <Button
              onClick={handleCopy}
              disabled={loading}
              className="bg-primary hover:brightness-110 text-primary-foreground font-medium rounded-xl px-6 h-11 text-xs flex items-center gap-2 cursor-pointer"
            >
              {copied ? (
                <>
